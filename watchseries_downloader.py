        ...
        if link.find("gorillavid.in") == -1:
            ...
            for i in urls:
                if i.find(".mp4") != -1:
                    ...
                    break
            if Ostype=="Windows":
                os.system("mkdir -p C:\Users\\"+getpass.getuser()+"\Downloads\watchseries\\" + s_name + "\Season-" + str(season)) 
                namel = "C:\Users\\"+getpass.getuser()+"\Downloads\watchseries\\" + s_name + "\\Season-" + str(
                season) + "\\" + s_name + "_S" + str(season) + "E" + str(episold) + "-" + name + ".mp4"
            elif Ostype=="Linux":
                os.system("mkdir -p /home/"+getpass.getuser()+"/Downloads/watchseries/" + s_name + "/Season-" + str(season)) 
...
def watchseries(link,start_season=0,start_episold=0):
    print "enter watchseries"
    wait_for_internet()
    s_name = link.split("/")[-1]
    print "\n"+s_name.replace("_"," ")
...
def datamining(link,s_name,start_season,start_episold):
    ...
    try:
        ...
        return
    doc = lh.fromstring(a.text)
    epview = doc.xpath('//a[@title="Unwatched episode"]/@href')
    print epview
    threads=list()
    for i in range(50):
        for j in range(200):
