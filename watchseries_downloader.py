...
-dmt to give threads for data mining 15
-dot to give threads for downloading 2
--main to give runtime inputs and download one episode
'''
import atexit
import base64
import json
...
class openload:
    ...
    def __init__(self, links):
        ...
        links = temp
        for i in links:
            i = i.replace("https://openload.co/f/", "https://17wy0ps.oloadcdn.net/dl/l/7Bsr1UUKomQ/")
            self.linklist.append(i)
...
def Run_process(exe, namel, season, episold, s_name):
    ...
    while True:
            ...
            if temp.find("skipping") == -1:
                ...
                if a != [""]:
                    ...
                    print("\033[K\033[07m" + "Downloaded: " + a[0] + "B Completed: " + a[1] + " Speed: " + a[
                        2] + "B\s Time : " + a[3] + " episold: " + "S_" + str(season) + "E_" + str(
                        episold) + "\033[0m \r"),
        ...
        if abc == 0 and final.find("100%") != -1:
            ...
            print "S_" + str(season) + "E_" + str(
                episold) + "\n-------------------------------Completed----------------------------------"
            notification_complete = notification_complete + s_name + 'Season ' + str(season) + " Episold " + str(
                episold) + "\n"
        ...
        if (try1 == 4):
            ...
            if "-l" in sys.argv:
                try:
                    ...
                    out = Run_process('wget  -c --limit-rate=' + str(speed) + 'k -O "' + namel + '" ' + urls[0], namel,
                                      season, episold, s_name)
                    if out.find("100%") != -1 or out.find("416 Requested Range Not Satisfiable") != -1:
                        print "abc"
                        for j in s_names:
                            if j[0] == s_name:
                                j[1] = season
                        ...
                        if j >= start_episold:
                            ...
                            if i <= final_season:
                                if j <= final_episold:
                                    ...
                                    threads.append(threading.Thread(target=leve1, args=(
                                        "http://thewatchseries.to" + x, i, j, s_name, 1)))

    index = 1
    sublist = list()
    threadnumber = 15
    if "-dmt" in sys.argv:
        threadnumber = sys.argv[sys.argv.index("dmt") + 1]
    for a in threads:
        if index % threadnumber == 0:
            for i in sublist:
                i.join()
            sublist = list()
        a.start()
        ...
        if a.find("watchseries") != -1:
            ...
            if a.find("/episode/") != -1 and len(a.split("/")) == 5:
                ...
                if raw_input("do u want to start form the top (y or n)").lower() == "y":
                    ...
                    watchseries(a, int(raw_input("enter the sesaon number")),
                                int(raw_input("enter the Episold number")))
            else:
